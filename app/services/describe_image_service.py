...
class GracefulContentScorer:
    ...
    def score(self, reference, response):
        ...
        # For image description, be more lenient with extra words
        # If response has more terms than reference, don't penalize heavily
        if len(resp_terms) > len(ref_terms):
            # Boost score slightly for comprehensive descriptions
            bonus = min(5, (len(resp_terms) - len(ref_terms)) * 0.5)
            semantic_overlap = min(100, semantic_overlap + bonus)
        ...
        return {
            ...
            "extra_terms_bonus": round(bonus, 2) if len(resp_terms) > len(ref_terms) else 0
        }
...
def evaluate_describe_image(reference_text, file, upload_folder):
    ...
    try:
        ...
        # === SPEAKING SCORE ===
        speaking = round(((fluency * 80) / 100) + ((pronunciation * 20) / 100), 2)
    finally:
        os.remove(tmp_path)
    return {
